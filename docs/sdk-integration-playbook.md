...
3. Автоматические `impression`-события через `impressionRef` (когда элемент реально видим), и ручной `track(...)`.
...
## 10. Текущая семантика runtime (v0.1.3)
...
- `impression` (автоматически из SDK при `enabled=true`, только когда элемент через `impressionRef` реально видим во viewport >= 50% не менее ~700 мс)
...
### 10.6 Нормализация аналитики по текущим вариантам

Backend нормализует разрез по вариантам к текущей конфигурации тоггла.

Текущее поведение:
- в аналитике по вариантам показываются только те `variant_key`, которые сейчас допустимы для этого тоггла;
- для булевого тоггла без вариантов в аналитике остается только вариант `on`.
